...
bloodOrangeSegments fruits = sum[seg fruit |fruit<-fruits, isBloodOrange fruit]
  where
  seg (Orange _ segments) = segments
-- 3.
worms :: [Fruit] -> Int
worms fruits = sum [1 | fruit<-fruits, appleWorm fruit]
 where appleWorm (Apple _ True) = True
       appleWorm (Apple _ _) = False
       appleWorm (Orange _ _) = False
...
eval e (p :&: q)      =  eval e p && eval e q
eval e (p :->: q)     =  (eval e p == eval e q) || ((eval e p==False) && (eval e q==True))
eval e (p :<->: q)    =  eval e p == eval e q  
...
equivalent p q = [ eval e p | e <- envs (nub(names p++names q)) ] == [ eval e q | e <- envs (nub(names p++names q)) ]
...
subformulas (F) = [(F)]
subformulas (T) = [(T)]
subformulas (Not p) = nub(subformulas p) ++ [(Not p)]
subformulas (p:|:q) = nub(subformulas p ++ subformulas q) ++ [(p:|:q)]
subformulas (p:&:q) = nub(subformulas p ++ subformulas q) ++ [(p:&:q)]
subformulas (p:->:q) = nub(subformulas p ++ subformulas q) ++ [(p:->:q)]
subformulas (p:<->:q) = nub(subformulas p ++ subformulas q) ++ [(p:<->:q)]
subformulas (p) = [p]
