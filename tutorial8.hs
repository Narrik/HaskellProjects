...
delta fsm state tran = [c | (a,b,c)<-(trans fsm),a==state,b==tran]
...
acceptsFrom m s [] = s `elem` final m
acceptsFrom m s (x:xs)  = or [acceptsFrom m d xs | d<-(delta m s x)]   
...
ddelta fsm state tran = canonical $ concat[delta fsm s tran | s<-state]


-- 6.
next :: (Ord q) => FSM q -> [[q]] -> [[q]]
next fsm xs = canonical([ddelta fsm x a |x<-xs, a<- alph fsm] ++ xs)


-- 7.
reachable :: (Ord q) => FSM q -> [[q]] -> [[q]]
reachable fsm xs | (next fsm xs) == next fsm (next fsm xs) =  next fsm xs
                 | otherwise = reachable fsm (next fsm xs)


-- 8.
dfinal :: (Ord q) => FSM q -> [[q]] -> [[q]]
dfinal fsm xs = [x |x<-xs, f<-final fsm,f `elem` x] 


-- 9.
dtrans :: (Ord q) => FSM q -> [[q]] -> [Transition [q]]
dtrans fsm xs =[(x,a,(ddelta fsm x a)) | x<-xs, a <-alph fsm] 


-- 10.
deterministic :: (Ord q) => FSM q -> FSM [q]
deterministic fsm = (reachable fsm [[start fsm]],alph fsm,[start fsm], dfinal fsm (reachable fsm [[start fsm]]),dtrans fsm (reachable fsm [[start fsm]]))
